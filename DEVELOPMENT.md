...
自 2.7.0 起首页顶栏的标题组替换为搜索框。输入关键词后按键盘搜索键发起搜索，
结果会替换海报网格展示；点击任意分类标签即可重新加载对应分类并退出搜索结果。
搜索范围始终是全部 `Movie` 与 `MusicVideo`，与当前所在分类无关。关键词为空时
仅提示，不发起请求。
...
- “全部”分类：直接在已加载的完整媒体库列表中随机挑选；
- “收藏”分类：自 2.7.1 起只在收藏影片范围内随机，直接使用当前已加载的收藏列表，
  收藏为空时仅 Toast 提示，不发起播放，也不回退到全局随机；
- 其他分类（含最近播放、最近入库和合集内容）：先向服务器请求完整媒体库再随机，
  与当前分类内容无关。加载期间按钮防重复点击，自 2.7.1 起加载过程不再弹出
  “正在从全部媒体库随机挑选”提示，加载完成后直接开始播放，失败时仍 Toast 提示。
...
重命名按钮弹出对话框并预填当前名称。自 2.7.1 起输入框支持自动换行，最多同时
显示四行，超长名称可在框内上下滚动查看，不再需要横向拖动；保存时内容中的换行
会被替换为空格。确认后应用先获取完整条目，替换 `Name`
...
搜索使用相同接口，并额外携带按 URL 编码的关键词，按名称升序返回匹配结果：
